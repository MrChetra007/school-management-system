...
| `/admin/users` | User management — tabs: Admin / Teacher / Librarian. Create, edit role, reset password, deactivate/reactivate accounts |
...
- [ ] Setup Supabase Edge Function `manage-user` (create/reset/delete auth users using service_role key)
- [ ] Setup `.env` file (Supabase URL + anon key + service_role key for edge function)
...
- [ ] Each tab shows a table of users with: name, email, status badge (active/inactive), actions
- [ ] **Create user** modal form:
  - Full name, email, password, role (select)
  - Submit → calls Supabase Edge Function `manage-user` with action `create`
  - Edge function creates `auth.users` entry (needs service_role key)
  - Trigger auto creates `public.users` with correct role
  - If role = `teacher` → also creates row in `teachers` table
- [ ] **Edit role** — dropdown to change role → calls `admin_update_user_role()` DB function
- [ ] **Reset password** — admin enters new password → calls Edge Function `manage-user` with action `reset_password`
- [ ] **Deactivate** — toggle active/inactive → calls `admin_update_user_status()` DB function:
...
- [ ] Status badge: 🟢 active / 🔴 inactive shown in table
